...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.episode_limit = map_params["limit"] # 200
        ...
        assert max_agent_id >= self.n_agents and max_agent_id >= self.n_enemies # hack
        # all of our test smac envs have <= args.max_agent_id enemies, such that all of them can use the same action space for multi-task training

        # if (
        #     map_params["map_type"] in ["MMM", "terran_gen"]
        #     and self.n_agents > self.n_enemies
        # ):
        #     self.n_actions = self.n_actions_no_attack + max_agent_id
        # else:
        self.n_actions = self.n_actions_no_attack + max_agent_id
    ...
    def step(self, actions):
        ...
        try:
            ...
            return self.get_obs(), self.get_state(), [[0]]*self.n_agents, [True for _ in range(self.n_agents)], infos, available_actions
        ...
        return self.get_obs(), self.get_state(), rewards, [terminated for _ in range(self.n_agents)], infos, available_actions
        ...
        for agent_id in range(self.n_agents):
            ...
            for idx in range(self.n_agents):
                if idx != agent_id:
                    ally_ids.append(idx)
            # ally
            ally_info = np.concatenate((state_dict["allies"][ally_ids], state_dict["last_action"][ally_ids]), axis=-1)
            # move
            unit = self.get_unit_by_id(agent_id)
            move_feats = np.zeros(move_feats_dim, dtype=np.float32)
            avail_actions = self.get_avail_agent_actions(agent_id)
            ...
            if self.obs_terrain_height:
                move_feats[ind:] = self.get_surrounding_height(unit)
            # own
            own_info = np.concatenate((state_dict["allies"][agent_id], state_dict["last_action"][agent_id]), axis=-1)
            state = np.concatenate([ally_info.flatten(), state_dict["enemies"].flatten(), move_feats, own_info])
            ...
            state_lt.append(state)

        # print(state_dict["allies"].shape, state_dict["enemies"].shape, state_dict["last_action"].shape)
        # (7, 7) (7, 6) (7, 13)
    ...
    def get_obs_size(self):
        ...
        return [all_feats, [n_allies, n_ally_feats-self.n_actions, self.n_actions], [n_enemies, n_enemy_feats], [1, move_feats], [1, own_feats-self.n_actions, self.n_actions, agent_id_feats, timestep_feats]]
    ...
    def get_state_size(self):
        ...
        return [size, [self.n_agents - 1, nf_al, self.n_actions], [self.n_enemies, nf_en], [1, move_feats], [1, nf_al, self.n_actions, agent_id_feats, timestep_feats]]
