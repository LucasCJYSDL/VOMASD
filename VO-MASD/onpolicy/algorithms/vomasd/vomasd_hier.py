...
class VOMASDHier(R_MAPPO):

    def __init__(self,
                 args,
                 policy,
                 device=th.device("cpu")):
        super(VOMASDHier, self).__init__(args, policy, device)
    ...
    def _forward_vae(self, batch, task, vae_training):
        ...
        btm_z_e = skill_embs.reshape(bs, proc_seq_len, n_agent, self.skill_dim)
        
        z_q, top_diff, btm_diff, group, log_group, v_out, emb_rwd = self.policy.forward_code(states[:, :proc_seq_len], btm_z_e, task, test_mode=vae_training)
        # torch.Size([32, 25, 3, 10]) torch.Size([800, 3, 1]) torch.Size([800, 3, 1]) torch.Size([800, 3, 1]) torch.Size([800, 3, 1])
        emb_loss = ((top_diff + btm_diff) * masks[:, :proc_seq_len].reshape(-1, n_agent, 1)).sum() / masks[:, :proc_seq_len].sum()
