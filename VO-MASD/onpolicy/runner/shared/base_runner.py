...
class Runner(object):
    ...
    def save_with(self, win_rate):
        """Save policy's actor and critic networks."""
        pre = str(int(win_rate * 1000))
        policy_actor = self.trainer.policy.actor
        torch.save(policy_actor.state_dict(), str(self.save_dir) + "/{}_actor.pt".format(pre))
        policy_critic = self.trainer.policy.critic
        torch.save(policy_critic.state_dict(), str(self.save_dir) + "/{}_critic.pt".format(pre))
        if self.trainer._use_valuenorm:
            policy_vnorm = self.trainer.value_normalizer
            torch.save(policy_vnorm.state_dict(), str(self.save_dir) + "/{}_vnorm.pt".format(pre))
